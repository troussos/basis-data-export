...
class CaloriesTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testConstruct()
    {
        ...
        $calories = new Calories($startTime, $interval, $rawArray);

        //Validate the basic assessment parameters
        $this->assertEquals('Calories Burned', $calories->getUnits());
        $this->assertEquals(80.5, $calories->getMin());
        $this->assertEquals(110, $calories->getMax());
        $this->assertEquals(567, $calories->getSum());
        $this->assertEquals(85, $calories->getAvg());
        $this->assertEquals(4.4, $calories->getStdev());

        //Grab the Raw Value Array from the object
        $values = $calories->getSummary();
